...
#define TCP_S_C             "TCPC"
#define IP_TO_CONNECT       "192.168.1.168"
...
void test_wifi(void)
{
    ...
    /* set the mode of wifi module for working  AT+WMODE=AP+STA */
    AT_cmd = hc22_cmds[HC22_AT_SET_WIFI_MODEL].at_cmd;
    pi_wifi_at_cmd(&wifi, AT_cmd, (void *) rx_buffer, 1);
    printf("---(%s)---WIFI response: %s\n", AT_cmd,rx_buffer);
    printf("\n\n");
    ...
    AT_cmd = AT_SOCK ;                              //"AT+SOCK=TCPC,192.168.1.164,8080"; 
    pi_wifi_at_cmd(&wifi, AT_cmd, (void *) rx_buffer, 1);
    printf("---(%s)---WIFI response: %s\n", AT_cmd,rx_buffer);
    printf("\n\n");
