...
static uint16_t buff_x_off =  10;
static uint16_t buff_y_off =  10;
...
/*
320x240 crop buff to 128*160 lcd .
*/
void crop_buff_for_lcd(uint16_t* buff_imag,uint16_t* lcd_buff,uint16_t buff_width,uint16_t buff_high  ,uint16_t lcd_width, uint16_t lcd_high,  uint16_t buff_x_off, uint16_t buff_y_off)
{

	uint16_t crop_buf_x = 0;
	uint16_t crop_buf_y = 0;
	...
	for(uint16_t buff_y=0; buff_y < buff_high; buff_y++ )
	{
		for(uint16_t buff_x=0; buff_x < buff_width; buff_x++)
		{
			...
			{
				if(((crop_buf_x = buff_x - buff_x_off) < lcd_width) && ( (crop_buf_y = buff_y - buff_y_off) < lcd_high ))	
				{
					*(lcd_buff + (crop_buf_y) * lcd_width + (crop_buf_x) ) = color_MSB_to_LSB (*( buff_imag + (buff_y) * buff_width + (buff_x) ) );
				}		
			}
		}
    }
...
/*
diplay memory size > lcd display memory size.
*/
	...
	for(uint16_t buff_y=0; buff_y < buff_high; buff_y++ )
	{
		for(uint16_t buff_x=0; buff_x < buff_width; buff_x++)
		{
			if((buff_x >= buff_x_off) && (buff_y >= buff_y_off))
			{
				...
				{
					writeFillRect(&lcd,(buff_x - buff_x_off),(buff_y - buff_y_off),1,1, ( *(buff_imag + (buff_y * buff_width) + buff_x ) ) );
				}		
			}
		}
	}
...
void test_camera_with_lcd(void)
{
    ...
    while (1)
    {
        ...
        crop_buff_for_lcd(imgBuff0,lcd_buff,CAMERA_WIDTH, CAMERA_HEIGHT,LCD_WIDTH,LCD_HEIGHT,buff_x_off,buff_y_off);
