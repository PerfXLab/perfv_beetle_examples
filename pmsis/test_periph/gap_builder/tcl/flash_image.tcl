...
proc gap_flasher_ctrl {ImageName ImageSize flash_offset sector_size flash_type} {
	...
	set device_struct_ptr_addr 0x1c000090
    set count [expr 0x0]
...
proc gap_flash_raw_spi {image_name image_size gap_tools_path board_name} {
	# flash the flasher
    puts "--------------------------"
    puts "begining flash session (spiflash)"
    puts "--------------------------"
    puts "load flasher to L2 memory"
	gap8_jtag_load_binary_and_start ${gap_tools_path}/gap_bins/gap_flasher@${board_name}.elf elf
	sleep 2
	# flash the flash image with the flasher
    puts "Instruct flasher to begin flash per se"
